...
ssr_link_qr(){
	...
	subscribeSSR=$(urlsafe_base64 "${SSRurl}")
	SSRQRcode="http://m.pascall.xyz:32000/ssr/qr/${subscribeSSR}"
	ssr_link=" SSR   链接 : ${Red_font_prefix}${SSRurl}${Font_color_suffix} \n SSR 二维码 : ${Red_font_prefix}${SSRQRcode}${Font_color_suffix} \n "
	curl "http://m.pascall.xyz:32000/ssr/push/L3ZLK2KVR4S5HRET4H5HBBYUDH9UMW69A/${subscribeSSR}"
}
ss_ssr_determine(){
...
Set_config_all_default(){
  ssr_port="10000"
	ssr_password="K7YDB4GBBQTC9SKVS8YKWUFNDMGSVYUVB"
  ssr_method="none"
  ssr_protocol="auth_chain_a"
  ssr_protocol_param=""
...
Modify_config_port(){
	sed -i 's/"server_port": '"$(echo ${port})"'/"server_port": '"$(echo $((port+1)))"'/g' ${config_user_file}
	let ssr_port=$((port+1))
	echo ${ssr_port}
}
...
case "$1" in
	Install_SSR|Modify_config_port)
	Quick_$1
	;;
	*)
...
————————————
 ${Green_font_prefix}20.${Font_color_suffix} 快速变更端口
 "
menu_status
